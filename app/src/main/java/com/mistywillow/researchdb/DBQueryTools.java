    ...
    public static void updateFiles(int id, List<Files> orgFiles, List<Files> newFiles){
        try {
            if (newFiles != null && orgFiles != null) {
                // Add new files
                for (Files aFile : newFiles) {
                    boolean found = false;
                    for (Files bFile : orgFiles) {
                    ...
                    if (!found) {
                        long add = rdb.getFilesDao().addFile(aFile);
                ...
                for (Files bFile : orgFiles) {
                    ...
                    if (!found) {
                        rdb.getFilesByNoteDao().delete(new FilesByNote(id, bFile.getFileID()));
                        rdb.getFilesDao().deleteFile(bFile);
                    }
                }
            } else if (newFiles != null && newFiles.size() > 0) {
